package java09;
/**
 * 演習９−３で作成した（Accountクラス）をテストするクラス
 */
...
public class E09_03 {
	...
	public static void main(String[] args) throws IOException {
		try {
			...
			//createAccountメソッドで口座インスタンス生成してコレクションに格納(開設時に預金しない) 引数は1（預金する）以外の正の整数
	...
	/**
	 * @brief 
	 * long型の引数を２つ受け取り、 その間の整数値が入力されるまでチェックするメソッド
	 *
	 * @param 
	 * fromNum 値(from) 
	 * toNum 値(to) 
	 ...
	 *
	 * @return 
	 * long checkedNum チェックされた範囲内の値を返却する
	 *
	 * @note
	 */
	/* ====================================================================== */
	...
	/**
	 * @brief 
	 * long型の基準値と符号を示す値を受け取り、 基準値以上、または基準値以下の実数値が入力されるまでチェックするメソッド
	 *
	 * @param 
	 * long referenceValue 基準値 
	...
	/**
	 * @brief 
	 * 口座クラスのインスタンスを生成するメソッド
	 *
	 * @param yesOrNo 預金するかしないかを示す整数（yes ...１　no ・・・1以外）
	 *
	 * @return 
	 * account 生成したインスタンス
	 *
	...
	static Account createAccount(int yesOrNo) throws IOException{
		...
		if(YES == yesOrNo){
			...
			Account account = new Account(
					...
					//預金残高を設定（１円以上のlong整数）
					moreOrLessCheck((long)1, 1, INS_ACBAL),
					//口座開設日を入力
					new Day(inputRangeCheck(1, 9999, INS_DOMINO) , inputRangeCheck(1, 12, INS_MONTH) , inputRangeCheck(1, 31, INS_DATE)),
					//口座種別を入力する
