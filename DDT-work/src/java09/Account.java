 ...
 * 
 * @author masahiro
 * 銀行口座クラス(p277）を作成し、口座開設日のフォールドと
 * toStringメソッドを追加すること。
 * また、コンストラクタや、ゲッタメソッドも追加すること
 *
 */
...
public class Account {
	...
	Account(String acName , int acNo , Day opeingDay , int typeOfAc){
		//口座名義を代入
		this.acName = acName;
		//口座番号(6桁）を代入
		this.acNo = acNo;
	...
	//開設と同時に預金をするコンストラクタ
	Account(String acName , int acNo , long acBalance , Day opeingDay , int typeOfAc){
		//口座名義を代入
		this.acName = acName;
		//口座番号(6桁）を代入
		this.acNo = acNo;
	...
	public String toString() {
		...
		//口座開設日、文字列表現にして返却
		return String.format("%04d年%02d月%02d日(%s)",opeingDay.getYear() ,opeingDay.getMonth(),opeingDay.getDate(),WD_JA[opeingDay.dayOfWeek()]);
	}
