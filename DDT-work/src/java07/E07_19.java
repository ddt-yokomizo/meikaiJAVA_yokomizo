package java07;
/**
 * 配列から指定の複数のインデックスの要素を削除するメソッドを作成せよ。
 * 削除後、後方に位置するインデックスを削除した要素分前にずらすこと。
 * また、削除後の残った要素の値は、変更しなくてもよい
 */
...
public class E07_19 {
	...
	/**
	 * @brief
	 * int型の引数を１つと符号を意味する値を受け取り、
	 * 基準値以上、または基準値以下の整数値が入力されるまでチェックするメソッド
	 *
	 * @param 
	...
	/**
	 * @brief
	 * リストとキーを受け取り、キーに対応する要素から指定数の要素を削除して表示
	 *
	 * @param 
	 * ArrayList<Integer> inputList 処理するリスト
	 ...
	 *
	 * @note
	 *  削除は、後方に位置するインデックスを前にずらして行うこと。
	 *  また、削除後に残った要素の値は、変更しなくてもよい
	 */
	/* ====================================================================== */
	static void arrayRemoveSome(ArrayList<Integer> inputList , int delIdx , int howMany){
		
		System.out.println("削除後の配列を表示します");
		//配列の要素を順番に確認し、指定のインデックスの要素を削除して表示
		for(int i = 0 ; i < inputList.size() ; i++){
			//指定のインデックス以降、指定回数分要素を削除する
			if((delIdx - 1) <= i && i <= ((delIdx - 1) + howMany)){
				//指定のインデックスから削除要素分周回して要素の作男処理を行う
				for(int j = i ; j < ((delIdx - 1) + howMany) ; j++){
					//要素削除後に前に詰められる要素があれば詰める
					//削除した位置より後に余る要素がなければ要素の削除は行わない(前に詰められないため）
					if((j + howMany) < inputList.size()){
						inputList.set(j, inputList.get(j + howMany));
					}
					//削除後の配列を表示
					System.out.println(inputList.get(j));
					//整合性を取るため削除中もループカウンタを回す
					i++;
				}
			}
			//削除しなかった、または前に詰められない要素はそのまま出力する
	...
	public static void main(String[] args) {
		try {
			...
			System.out.println("リストから指定の要素を削除します。");
			...
			//inputRangeCheckメソッドから1番目〜リストの要素数までの整数値を入力させて削除する要素の開始位置を決定する
			selectIdx = inputRangeCheck(1, inputList.size(), "何番目の要素を削除しますか？＞");
			
			//inputRangeCheckメソッドから1個〜指定できるリストの要素限界までの整数値を入力させて、合計で幾つ要素を削除するか決定する
			howMany = inputRangeCheck(1, (inputList.size() - (selectIdx - 1)), "幾つ要素を削除しますか？＞");

			//arrayRemoveSomeメソッドで指定の要素を削除してリストを表示
			arrayRemoveSome(inputList, selectIdx , howMany);
		} finally {
			// scannerをクローズ
			scanner.close();
		}
