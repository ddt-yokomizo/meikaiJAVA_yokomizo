 ...
 * また、受け取る行数および列数は同一のものを受け取ることを前提としてもよい
 */
...
public class E07_28 {
	...
	public static void main(String[] args) {
		try {
			...
			//addMatrixメソッドへ２つの２次元配列を渡して、要素の合計を取得し、さらにviewListOf2dimentionメソッドで要素を出力する
			viewListOf2dimention(addMatrix(arrayListX, arrayListY));
		} finally {
			// scannerをクローズ
			scanner.close();
		}
	 ...
	 */
	/* ====================================================================== */
	static int[][] addMatrix(int[][] listOf2dimentionX , int[][] listOf2dimentionY) {

		//加算結果を格納する２次元配列を宣言（前提として行数はlistOf2dimentionXと同一）
		int[][] listOf2dimentionZ = new int[listOf2dimentionX.length][];
		
		for(int i = 0 ; i < listOf2dimentionX.length ; i++){
			//加算結果を格納する２次元配列の読み込み中行の列数をnewする（前提として列数はlistOf2dimentionXと同一）
			listOf2dimentionZ[i] = new int[listOf2dimentionX[i].length];
			//ループして配列Zに行列を加算する
