...
/**
 * 
 * @author masahiro コマンドライン引数で指定された月のカレンダーを表示するプログラムを作成せよ。 要件
 *         年のみが与えられた場合はその年の1月から12月までのカレンダーを表示 年と月が与えられた場合は、その月のカレンダーを表示
...
public class E15_08 {
	...
	static final LocalDate sLOCAL_DATE = LocalDate.now();
	...
	public E15_08(int year, int month) throws IllegalArgumentException {
		// 多重定義されたコンストラクタから年を更新
		this(year);

		// 引数の月が１〜１２でなければ例外IllegalArgumentExceptionをスロー
		if (month < 1 || 12 < month) {
			throw new IllegalArgumentException("入力された月が不正です。");
		}
	...
	public static void showCalender(int year, int month) {
		...
		int dayOfWeek; // 曜日を示す変数 0 ・・・ 日曜、1 ・・・ 月曜、2 ・・・ 火曜、3 ・・・ 水曜 ,4 ・・・
						// 木曜、5 ・・・ 金曜、6 ・・・ 土曜、
	 ...
	 * @param
	 *
	 * @return 曜日を表す0~6のint型整数
	 *
	 * @note 当メソッドは「ツェラーの公式」をそのまま流用したものであるため、 処理内容についてコメントは省略する（テキストの指示通り）
	 * 
